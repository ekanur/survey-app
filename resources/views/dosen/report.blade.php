...
<div class="row">
				...
				<table class="table mb-0" id="datatable1">
					...
					<tbody>
						@php
							...
							$i=0;
						@endphp
						@foreach($list_pemahaman_vmts as $list_pemahaman_vmts)
						<tr>
							<th>{{ ucfirst($key[$i++]) }}</th>
							<td>
								{{ $list_pemahaman_vmts['Ya'] }}
							</td>
							<td>
								{{ $list_pemahaman_vmts['Tidak'] }}
							</td>
						</tr>
						@endforeach
					</tbody>
	...
	<div class="col-md-6 col-sm-12 mb-4">
		@component("chart.column")
			...
			@slot("subjudul_chart") Prodi & Fakultas @endslot
			@slot("judul_y") Total @endslot
			@slot("tipe_value") number @endslot
		@endcomponent
	</div>
